...
XYZ_PATH   = "/scratch/rasera/PyISV/structures/full_min_ptmd_m18_nCu_0.xyz"
LABEL_FILE = "/scratch/rasera/PyISV/Ag38_labels/combined_isv_labels_2D_nonMin_to_min_k15_nCu_0.txt"
...
print(f"Min label: {torch.min(labels)}")
print(f"Max label: {torch.max(labels)}")

assert torch.min(labels) >= 0 and torch.max(labels) < NUM_CLASSES, \
    f"Labels must be between 0 and {NUM_CLASSES - 1}, found label(s) out of bounds."
