...
class Classifier2D(nn.Module):
    ...
    def __init__(self, embed_dim, flat_dim, num_classes=14, num_encoder_final_channels=16):
        ...
        self.encoder = Autoencoder2D(
            embed_dim=embed_dim,
            flat_dim=flat_dim,
            num_encoder_final_channels=num_encoder_final_channels
        ).encoder

        # Classification head
        self.fc1 = nn.Linear(flat_dim, embed_dim)
        self.relu = nn.ReLU()
    ...
    def forward(self, x):
        # x: (batch, 1, H, W)
        z = self.encoder(x)                 # (batch, C, h, w)
        z = z.view(z.size(0), -1)           # (batch, flat_dim)
        z = self.relu(self.fc1(z))          # bottleneck
        z = self.dropout(z)
        logits = self.fc2(z)                # (batch, num_classes)
        return logits
...
class Autoencoder2D(nn.Module):
    ...
    def __init__(self, embed_dim, flat_dim, num_encoder_final_channels=16):
        ...
        self.encoder = nn.Sequential(
            ...
            nn.ReLU(),
            nn.BatchNorm2d(32),
            nn.Conv2d(in_channels=32, out_channels=self.num_final_ch, kernel_size=3, padding=1),
            #nn.MaxPool2d(kernel_size=2, stride=2),
            nn.ReLU(),
        ...
        self.decoder = nn.Sequential(
            ...
            nn.ConvTranspose2d(in_channels=self.num_final_ch, out_channels=32, kernel_size=3, padding=1),
            nn.ReLU(),
            nn.BatchNorm2d(32),
            #nn.Upsample(scale_factor=2),
            ...
            nn.BatchNorm2d(64),
            nn.Upsample(scale_factor=2),
            nn.ConvTranspose2d(in_channels=64, out_channels=128, kernel_size=3, padding=0),
            nn.ReLU(),
            nn.BatchNorm2d(128),
            nn.Upsample(scale_factor=2),
            nn.Conv2d(in_channels=128, out_channels=1, kernel_size=3, padding=0)
        )
